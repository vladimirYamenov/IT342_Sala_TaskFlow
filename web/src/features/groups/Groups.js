...
export default function Groups({ user, addToast }) {
  ...
  const handleLeaveGroup = async (groupId) => {
    try {
      await groupApi.removeMember(groupId, user.userId);
  ...
  if (selected) {
    const isSelectedAdmin = selected.members?.some(
    ...
    return (
      ...
      <div className="page-content fade-in">
        <div className="page-header">
          ...
          {!isSelectedAdmin && (
            <div>
              {confirmLeaveGroupId === selected.id ? (
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <span className="text-muted" style={{ fontSize: 14 }}>Leave this group?</span>
                  <button className="btn btn-danger btn-sm" onClick={() => handleLeaveGroup(selected.id)}>Yes, Leave</button>
                  <button className="btn btn-secondary btn-sm" onClick={() => setConfirmLeaveGroupId(null)}>Cancel</button>
                </div>
              ) : (
                <button
                  className="btn btn-secondary"
                  onClick={() => setConfirmLeaveGroupId(selected.id)}
            ...
            <div className="member-list">
              {(selected.members || []).map((m) => (
                <div className="member-row" key={m.userId}>
                  ...
                  {m.userId !== user?.userId && isSelectedAdmin && (
                    <button
                ...
                {isGroupAdmin ? (
                  ...
                  <>
                    {confirmLeaveGroupId === group.id ? (
                      <>
                        <button className="btn btn-sm btn-danger" onClick={() => handleLeaveGroup(group.id)}>Confirm Leave</button>
                        <button className="btn btn-sm btn-secondary" onClick={() => setConfirmLeaveGroupId(null)}>Cancel</button>
                      </>
                    ) : (
                      <button
                        className="icon-btn icon-btn-danger"
                        title="Leave group"
                        onClick={(e) => { e.stopPropagation(); setConfirmLeaveGroupId(group.id); }}
                      >🚪</button>
                    )}
                  </>
                )}
